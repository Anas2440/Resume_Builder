...
export class GeminiService {
  ...
  async generateResumeSuggestions(resume: string, jobDescription: string): Promise<string> {
      ...
      contents: [
        {
          parts: [
            {
              text: buildPrompt(resume, jobDescription)
            }
          ]
        }
      ],
    ...
    const endpoint = `${GEMINI_API_BASE}/${GEMINI_MODEL}:generateContent?key=${encodeURIComponent(this.apiKey)}`
    ...
    try {
      ...
      throw new Error(`Network error while contacting Gemini: ${error instanceof Error ? error.message : "Unknown network error"}`)
    }
